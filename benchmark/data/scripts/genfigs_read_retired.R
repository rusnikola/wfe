...
filenames<-c("hashmap","list","natarajan")
for (f in filenames){
...
lindata = rbind(fedatalin, hedatalin, rangenewdatalin, rcudatalin, hazarddatalin)
...
linchart<-ggplot(data=lindata,
  ...
  geom_line()+xlab("Number of Threads")+ylab("Number of Unreclaimed Objects")+geom_point(size=5)+
...
ggsave(filename = paste("../final/",f,"_linchart_retired_read.pdf",sep=""),linchart,width=8, height = 5.5, units = "in", dpi=300)
